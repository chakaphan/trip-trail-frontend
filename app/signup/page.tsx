...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-nature-green/5 via-sky-blue/5 to-light-brown/5 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-foreground/60">เริ่มต้นบันทึกทริปธรรมชาติของคุณ</p>
        </div>

        {/* Signup Card */}
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <h2 className="text-2xl font-bold text-foreground mb-6">สมัครสมาชิก</h2>
          ...
          <div className="space-y-3 mb-6">
            ...
            <button className="w-full flex items-center justify-center gap-3 px-6 py-3 border-2 border-foreground/10 rounded-lg hover:bg-foreground/5 transition-colors">
              ...
              <span className="font-medium">สมัครด้วย Apple</span>
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-start gap-2">
              <input 
                type="checkbox" 
                id="terms"
                className="mt-1 rounded border-foreground/20 text-nature-green focus:ring-nature-green" 
              />
              <label htmlFor="terms" className="text-sm text-foreground/70">
                ฉันยอมรับ{' '}
                <Link href="/terms" className="text-nature-green hover:text-nature-green/80">
                  เงื่อนไขการใช้งาน
                </Link>
                {' '}และ{' '}
                <Link href="/privacy" className="text-nature-green hover:text-nature-green/80">
            ...
            {success && (
              <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-green-600 text-sm">
                สมัครสมาชิกสำเร็จ! กำลังนำคุณไปยังหน้าแดชบอร์ด...
              </div>
            )}
          ...
          <p className="mt-6 text-center text-sm text-foreground/60">
            มีบัญชีอยู่แล้ว?{' '}
            <Link href="/login" className="text-nature-green font-semibold hover:text-nature-green/80">
              เข้าสู่ระบบ
            </Link>
          </p>
        </div>
